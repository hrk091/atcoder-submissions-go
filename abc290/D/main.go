...
func main() {
	...
	t := atoi(sc.Text())
	for i := 1; i <= t; i++ {
		a := scanLineInt(sc, 3, 0)
		solve(a[0], a[1], a[2])
	}
}

func solve(n, d, k int) {
	y := coyakusu(n, d)
	countInLoop := n / y
	// loop := n / countInLoop
	// firstLoop := make([]int, countInLoop)
	// for i := 1; i < countInLoop; i++ {
	// 	firstLoop[i] = (firstLoop[i-1] + d) % n
	// }
	// if debug > 0 {
	// 	fmt.Printf("firstLoop: %+v\n", firstLoop)
	// }
	ans := ((k-1)%countInLoop*d)%n + (k-1)/countInLoop
	fmt.Println(ans)
}

func coyakusu(nn, dd int) int {
	for {
		if nn > dd {
			nn = nn % dd
		} else {
