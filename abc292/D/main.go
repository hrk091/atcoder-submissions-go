...
func newUnionFind(numNodes int) *unionFind {
	...
	fillSlice(sizes, 1)
	edges := make([]int, numNodes+1)
	return &unionFind{parents, sizes, edges}
}
...
func (uf *unionFind) unite(u, v int) {
	...
	if uf.sizes[rootU] < uf.sizes[rootV] {
		...
		uf.edges[rootV] = uf.edges[rootU] + uf.edges[rootV] + 1
	} else {
		uf.parents[rootV] = rootU
		uf.sizes[rootU] = uf.sizes[rootU] + uf.sizes[rootV]
		uf.edges[rootU] = uf.edges[rootU] + uf.edges[rootV] + 1
	}
}
...
func (uf *unionFind) isRoot(u int) bool {
	return uf.parents[u] == -1
}

func (uf *unionFind) isMatch(u int) bool {
	return uf.sizes[u] == uf.edges[u]
}
