...
func main() {
	...
	if float64(a) > float64(b)*2/sqrt(3) {
		fmt.Println(float64(b) * 2 / sqrt(3))
		return
	}
	if float64(b) > float64(a)*2/sqrt(3) {
		fmt.Println(float64(a) * 2 / sqrt(3))
		return
	}
	delta := math.Pi / float64(36)
	inc := true
	cur := float64(0)
	count := 0
	for {
		...
		diff := float64(a)/math.Cos(cur) - float64(b)/math.Cos(math.Pi/6-cur)
		if debug > 0 {
			fmt.Printf("%d -> diff: %+v\n", count, diff)
		}
		if math.Abs(diff) < 1e-9 {
			ans := float64(a) / math.Cos(cur)
			fmt.Println(ans)
			return
		}
		if inc && diff > 0 {
