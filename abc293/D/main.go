...
func main() {
	...
	for i := 1; i <= m; i++ {
		...
		g.addEdge(ropeNum(i), as[i])
		g.addEdge(as[i], ropeNum(i))
		g.addEdge(ropeNum(i), cs[i])
		g.addEdge(cs[i], ropeNum(i))
	}
	if debug > 0 {
		fmt.Printf("g.data: %+v\n", g.data)
	}
	...
	counted := make([]bool, n+m+1)
	for i := 1; i < len(visited); i++ {
		if visited[i] {
			continue
		}
		if debug > 0 {
			fmt.Printf("next:%d, visited:%+v\n", i, visited)
		}
		total++
		g.dfs(i, visited, func(curr, next int) {
			...
			if visited[next] && !counted[curr] {
				if debug > 0 {
					fmt.Printf("circle found: curr=%d next=%d\n", curr, next)
				}
				circle++
				counted[next] = true
			}
