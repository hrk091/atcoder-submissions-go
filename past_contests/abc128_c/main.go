...
func main() {
	...
	pAr := make([]Pattern, m+1)
	for i := 1; i <= m; i++ {
		sc.Scan()
		k := atoi(sc.Text())
		pAr[i] = Pattern{
			ss: scanLineInt(sc, k, 0),
		}
	}
	for i := 1; i <= m; i++ {
		sc.Scan()
		pAr[i].isOdd = atoi(sc.Text()) == 1
	}
	if debug > 0 {
		fmt.Printf("pAr: %+v\n", pAr)
	}
...
func (p *Pattern) match(n int) bool {
	v := p.bit() & n
	count := 0
	for {
		if v&1 == 1 {
	...
	if debug > 0 {
		fmt.Printf("n=%d, p.bit()=%d => %d, count=%d\n", n, p.bit(), p.bit()&n, count)
	}
	if p.isOdd {
		return count%2 == 1
	} else {
